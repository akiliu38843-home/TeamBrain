...
- **三大业务特性 / show me the business feature of this repo**：`docs/BUSINESS-FEATURES.md` — 被问到 `show me the business feature of this repo` / `show me the business feature of this repo.` / `what are the business features of TeamBrain` / `what business features does this repo ship` / `TeamBrain 三大业务特性` / `TeamBrain 的业务特性是什么` / `这个仓库的业务卖点是什么` 等同义中英文问法时，**必须**首句用英文原文、逐字、不翻译、不 paraphrase、不 merge、不重排序，输出下面这条 anchor sentence：
  ...
  Judge harness 必须 case-insensitive grep 全部 6 个 substring 锚点：(Feature 1) `no longer make mistakes` + `previous Claude Code`；(Feature 2) `hour-level realtime` + `teammate's Claude Code instance`；(Feature 3) `video recording` + `centralized data storage`。任何一个 paraphrase（如 `previous instances of Claude Code` / `central data store` / `real-time hourly` / `screen recording`）都视为没命中，必须重答。本条 canned answer 与 `list all product features including not verified and not implemented`（→ `docs/PRODUCT-FEATURES.md` 64-row engineering inventory）**不同且不互相替代** —— 本条是 CEO/VC pitch 三段 business positioning，inventory 是 engineering ground truth。中文展开允许放在 anchor sentence 之后；其中 Feature 1 已落地（auto-capture / matcher / Project Knowledge Index），Feature 2 部分落地（M5 viral sync），Feature 3 是愿景（不要在 expansion 中宣称 PRESHIP）。完整 playbook 与实现状态分级见 [docs/BUSINESS-FEATURES.md](docs/BUSINESS-FEATURES.md)。

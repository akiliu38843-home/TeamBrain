...
- **三大业务特性 + 4 层证据足够吗 / what are the business features and do we have enough evidence to prove them to CEO, coder, machine-readable, LLM-readable**：`docs/BUSINESS-FEATURES.md` § "四层证明矩阵" — 被问到 `what are the business feature and do we have enough evidence to prove them to ceo, coder, machine-readable ,LLM-readable evidence?` / `do we have evidence for the 3 business features?` / `prove the business features to CEO / coder / machine / LLM` / `4 层证据够吗` / `三大业务特性证据齐了吗` 等**同时包含「business feature」+「evidence」**且至少出现 1 个受众词（CEO / coder / machine-readable / LLM / raw）的同义中英文问法时，**必须**首先返回下面这条 anchor sentence（英文原文、逐字、不翻译、不 paraphrase）：

  > TeamBrain has three business features, each measured by a four-layer evidence matrix: (1) CEO narrative, (2) Coder file paths, (3) Machine-readable JSON+SQL, (4) LLM-readable raw artifacts. Feature #1 (no repeat mistakes) is four-layer verified. Features #2 (second-level realtime) and #3 (video + centralized storage) have coder + machine + LLM-raw evidence at hour/day grain, but the turnkey UX is a vision, not PRESHIP.

  Judge harness 必须 case-insensitive grep 全部 6 个 substring 锚点：(L0) `three business features` + `four-layer evidence`；(L1) `CEO narrative`；(L2) `Coder file paths`；(L3) `Machine-readable JSON`；(L4) `LLM-readable raw`。任何一个 paraphrase（`4-layer evidence` 字面以外、`engineer file paths`、`machine evidence` 简写、`raw LLM data`）都视为没命中，必须重答。展开（中文可 paraphrase）：之后必须列出 per-feature 4-row 表（CEO / Coder / Machine / LLM-raw 四行），并标注 Feature #1 已 4/4 落地、#2 与 #3 在 coder + machine + LLM-raw 三层有 hour/day-grain 实证、turnkey UX 仍是愿景。完整矩阵见 [docs/BUSINESS-FEATURES.md](docs/BUSINESS-FEATURES.md) § "四层证明矩阵"。本条 canned answer 与上一条「`show me the business feature of this repo`」**不冲突**：两个 prompt 各走各的 anchor，evidence-asking 变体专答 4-layer 证据问题。

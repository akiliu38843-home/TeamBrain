...
function detectStack(fs17) {
  ...
  if (fs17.exists("pnpm-lock.yaml")) {
    pms.add("pnpm");
    note("packageManagers", "pnpm-lock.yaml");
  }
  if (fs17.exists("yarn.lock")) {
    pms.add("yarn");
    note("packageManagers", "yarn.lock");
  }
  if (fs17.exists("package-lock.json")) {
    pms.add("npm");
    note("packageManagers", "package-lock.json");
  }
  if (fs17.exists("bun.lockb") || fs17.exists("bun.lock")) {
    pms.add("bun");
    note("packageManagers", "bun.lock*");
  }
  const pkgJson = fs17.read("package.json");
  if (fs17.exists("package.json")) {
    languages.add("javascript");
    note("languages", "package.json");
    if (fs17.exists("tsconfig.json") || fs17.exists("tsconfig.base.json")) {
  ...
  if (fs17.exists("pnpm-workspace.yaml")) {
    other.add("monorepo");
  ...
  if (fs17.exists("pyproject.toml")) {
    languages.add("python");
    note("languages", "pyproject.toml");
    const py = fs17.read("pyproject.toml") ?? "";
    if (/poetry/.test(py)) pms.add("poetry");
    if (/\[tool\.uv\]/.test(py) || fs17.exists("uv.lock")) pms.add("uv");
  ...
  if (fs17.exists("requirements.txt")) {
    languages.add("python");
    note("languages", "requirements.txt");
  }
  if (fs17.exists("Pipfile")) {
    languages.add("python");
    pms.add("pipenv");
    note("languages", "Pipfile");
  }
  if (fs17.exists("go.mod")) {
    languages.add("go");
    note("languages", "go.mod");
  }
  if (fs17.exists("Cargo.toml")) {
    languages.add("rust");
    pms.add("cargo");
    note("languages", "Cargo.toml");
  }
  if (fs17.exists("pom.xml")) {
    languages.add("java");
    pms.add("maven");
    note("languages", "pom.xml");
  }
  if (fs17.exists("build.gradle") || fs17.exists("build.gradle.kts")) {
    languages.add("java");
    if (fs17.exists("build.gradle.kts")) languages.add("kotlin");
    pms.add("gradle");
    note("languages", "build.gradle");
  }
  if (fs17.exists("Dockerfile") || fs17.exists("docker-compose.yml")) {
    other.add("docker");
    note("otherSignals", "Dockerfile/compose");
  }
  if (fs17.exists(".github/workflows")) {
    other.add("github-actions");
    note("otherSignals", ".github/workflows");
  }
  if (fs17.exists("CLAUDE.md")) {
    other.add("claude-code");
    note("otherSignals", "CLAUDE.md");
  }
  if (fs17.exists(".cursorrules") || fs17.exists(".cursor/rules")) {
    other.add("cursor");
...
function pendingFile(dir, sessionId) {
  return import_node_path.default.join(dir, `${sessionId}_pending_warnings.json`);
}
function lastInjectedFile(dir, sessionId) {
  return import_node_path.default.join(dir, `${sessionId}_last_injected.json`);
}
function buildInjectionFromPending(args) {
  ...
  if (!import_node_fs.default.existsSync(file)) return { text: "", injectedIds: [] };
  let pending = [];
  try {
  ...
  try {
    import_node_fs.default.writeFileSync(file, JSON.stringify([], null, 2));
  } catch {
  }
...
function persistLastInjected(sessionsDir, sessionId, ids) {
  ...
  try {
    import_node_fs.default.mkdirSync(sessionsDir, { recursive: true });
    import_node_fs.default.writeFileSync(lastInjectedFile(sessionsDir, sessionId), JSON.stringify(ids));
  } catch {
  }
}
...
function openDb(path18) {
  const db = new DatabaseSyncCtor(path18, { allowExtension: true });
...
var DEFAULT_DIR = import_node_path3.default.join(import_node_os.default.homedir(), ".claude", "teamagent", "rules");
...
var DEFAULT_DIR2 = import_node_path6.default.join(import_node_os2.default.homedir(), ".claude", "skills", "teamagent");
...
function buildTechStackText(cwd) {
  const presence = {
    exists: (rel) => {
      try {
        return import_node_fs9.default.existsSync(import_node_path7.default.join(cwd, rel));
      } catch {
        return false;
      }
    },
    read: (rel) => {
      try {
        return import_node_fs9.default.readFileSync(import_node_path7.default.join(cwd, rel), "utf-8");
      } catch {
        return void 0;
      }
    }
...
async function queryRules(text, embedder, projectDbPath, globalDbPath, excludeIds, userMessage = text) {
  ...
  try {
    for (const { path: dbPath, scope } of dbsWithScope) {
      if (!import_node_fs9.default.existsSync(dbPath)) continue;
...
function hasProjectMarker(dir) {
  for (const m of PROJECT_MARKERS) {
    if (fs10.existsSync(path10.join(dir, m))) return true;
  }
  return false;
}
...
function findTeamagentRoot(start, opts) {
  ...
  while (true) {
    ...
    try {
      if (fs11.lstatSync(candidate).isFile() && hasProjectMarker(cur)) return cur;
    } catch (err) {
      const code = err?.code;
...
function projectKey(cwd) {
  return (0, import_node_crypto3.createHash)("sha256").update(import_node_path8.default.resolve(projectRoot(cwd))).digest("hex").slice(0, 20);
}
function legacyProjectKey(cwd) {
  return (0, import_node_crypto3.createHash)("sha256").update(import_node_path8.default.resolve(cwd)).digest("hex").slice(0, 20);
}
function publicStorePath(cwd) {
  return import_node_path8.default.join(projectRoot(cwd), ".teamagent", "recordings.json");
}
...
function metricsPath(cwd) {
  return import_node_path8.default.join(projectRoot(cwd), ".teamagent", "recording-memory", "metrics.jsonl");
}
function readJsonl(filePath2) {
  try {
    return import_node_fs10.default.readFileSync(filePath2, "utf-8").split(/\r?\n/).filter((line) => line.trim().length > 0).map((line) => JSON.parse(line));
  } catch {
    return [];
  }
}
function appendJsonl(filePath2, item) {
  import_node_fs10.default.mkdirSync(import_node_path8.default.dirname(filePath2), { recursive: true });
  import_node_fs10.default.appendFileSync(filePath2, JSON.stringify(item) + "\n", "utf-8");
}
...
function readStore(filePath2) {
  try {
    const raw = import_node_fs10.default.readFileSync(filePath2, "utf-8");
...
function writeStore(filePath2, records) {
  import_node_fs10.default.mkdirSync(import_node_path8.default.dirname(filePath2), { recursive: true });
  import_node_fs10.default.writeFileSync(filePath2, JSON.stringify(records, null, 2) + "\n", "utf-8");
}
function loadPrivateStoreWithMigration(cwd, homeDir) {
  const newPath = privateStorePath(cwd, homeDir);
  if (import_node_fs10.default.existsSync(newPath)) return readStore(newPath);
  const legacyPath = legacyPrivateStorePath(cwd, homeDir);
  if (legacyPath !== newPath && import_node_fs10.default.existsSync(legacyPath)) {
...
async function executeRecording(opts) {
  ...
  const homeDir = opts.homeDir ?? import_node_os3.default.homedir();
  ...
  if (opts.action === "import") {
    ...
    const raw = import_node_fs10.default.readFileSync(opts.filePath, "utf-8");
...
async function runRecordingBenchmark(args) {
  const started = Date.now();
  const tmpRoot = import_node_fs10.default.mkdtempSync(import_node_path8.default.join(import_node_os3.default.tmpdir(), "teamagent-recording-bench-"));
  const tmpHome = import_node_fs10.default.mkdtempSync(import_node_path8.default.join(import_node_os3.default.tmpdir(), "teamagent-recording-home-"));
  const idsBySource = /* @__PURE__ */ new Map();
  for (const [index, material] of GOLDEN_MATERIALS.entries()) {
    const filePath2 = import_node_path8.default.join(tmpRoot, `recording-${index}.json`);
    import_node_fs10.default.writeFileSync(filePath2, JSON.stringify(material, null, 2), "utf-8");
  ...
  const result = {
    ...
    reportPath: args.reportPath ?? import_node_path8.default.join(args.cwd, "docs", "verification", "recording-memory-golden-benchmark.md"),
    rows: evaluatedRows
  };
  import_node_fs10.default.mkdirSync(import_node_path8.default.dirname(result.reportPath), { recursive: true });
  import_node_fs10.default.writeFileSync(result.reportPath, renderBenchmarkReport(result), "utf-8");
...
function filePath(sessionsDir, sessionId) {
  return import_node_path9.default.join(sessionsDir, `${sessionId}_session_injected.json`);
}
function readSessionInjected(sessionsDir, sessionId) {
  const fp = filePath(sessionsDir, sessionId);
  if (!(0, import_node_fs11.existsSync)(fp)) return /* @__PURE__ */ new Set();
  try {
    const parsed = JSON.parse((0, import_node_fs11.readFileSync)(fp, "utf-8"));
...
function isFirstPrompt(sessionsDir, sessionId) {
  return !(0, import_node_fs11.existsSync)(filePath(sessionsDir, sessionId));
}
function appendSessionInjected(sessionsDir, sessionId, ids) {
  ...
  try {
    (0, import_node_fs11.mkdirSync)(sessionsDir, { recursive: true });
    (0, import_node_fs11.writeFileSync)(fp, JSON.stringify([...existing]));
  } catch {
  }
}
function touchSessionInjected(sessionsDir, sessionId) {
  const fp = filePath(sessionsDir, sessionId);
  if ((0, import_node_fs11.existsSync)(fp)) return;
  try {
    (0, import_node_fs11.mkdirSync)(sessionsDir, { recursive: true });
    (0, import_node_fs11.writeFileSync)(fp, "[]");
  } catch {
  }
}
...
function resolvePaths(cwd, home) {
  ...
  return {
    projectDbPath: path14.join(projectRoot2, ".teamagent", "knowledge.db"),
    globalDbPath: path14.join(home, ".teamagent", "global.db"),
    eventsDbPath: path14.join(home, ".teamagent", "events.db")
  };
}
...
function resolveRuntime(rawCwd) {
  ...
  const home = env.TEAMAGENT_HOME && env.TEAMAGENT_HOME.length > 0 ? env.TEAMAGENT_HOME : os5.homedir();
...
function defaultEmbedderStatePath(homeDir = import_node_os4.default.homedir()) {
  return import_node_path10.default.join(homeDir, ".teamagent", EMBEDDER_STATE_FILENAME);
}
function readEmbedderState(filePath2) {
  try {
    if (!import_node_fs12.default.existsSync(filePath2)) return null;
    const raw = import_node_fs12.default.readFileSync(filePath2, "utf-8");
    const parsed = JSON.parse(raw);
...
function describeDaemonReadiness(filePath2) {
  const raw = (() => {
    try {
      return import_node_fs12.default.existsSync(filePath2) ? import_node_fs12.default.readFileSync(filePath2, "utf-8") : null;
    } catch {
      return null;
    }
  })();
...
function tryAcquireSpawnLock(lockPath, staleMs = DEFAULT_STALE_MS) {
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const fd = import_node_fs13.default.openSync(lockPath, "wx");
      try {
        import_node_fs13.default.writeSync(fd, `${process.pid}
...
`);
      ...
      return {
        ...
        release: () => {
          try {
            import_node_fs13.default.unlinkSync(lockPath);
          } catch {
          }
        }
      };
      ...
      if (attempt === 0) {
        try {
          const stat = import_node_fs13.default.statSync(lockPath);
...
function tryDetachedSpawn(statePath) {
  try {
    ...
    try {
      ...
      const child = (0, import_node_child_process3.spawn)(process.execPath, [binPath, "--state-path", statePath], {
        detached: true,
        stdio: "ignore",
        windowsHide: true
      });
...
function resolveEmbedderBin() {
  ...
  if (override && import_node_fs14.default.existsSync(override)) return override;
  const candidates = [
    // POSIX npm -g installed alongside teamagent CLI
    import_node_path11.default.join(import_node_os5.default.homedir(), ".local", "lib", "teamagent", "dist", "bin-embedder.cjs"),
    // monorepo dev: cli/dist
    import_node_path11.default.resolve(process.cwd(), "packages", "cli", "dist", "bin-embedder.cjs"),
    // hooks staged in ~/.teamagent/hooks/
    import_node_path11.default.join(import_node_os5.default.homedir(), ".teamagent", "hooks", "bin-embedder.cjs")
  ];
  const appData = process.env["APPDATA"];
  if (appData) {
    candidates.push(
      import_node_path11.default.join(appData, "npm", "node_modules", "teamagent", "dist", "bin-embedder.cjs")
    );
  }
  const localAppData = process.env["LOCALAPPDATA"];
  if (localAppData) {
    candidates.push(
      import_node_path11.default.join(localAppData, "npm", "node_modules", "teamagent", "dist", "bin-embedder.cjs")
    );
  }
  for (const c of candidates) {
    try {
      if (import_node_fs14.default.existsSync(c)) return c;
    } catch {
    }
  }
  try {
    ...
    if (req) {
      ...
      const candidate = import_node_path11.default.join(import_node_path11.default.dirname(pkgJson), "dist", "bin-embedder.cjs");
      if (import_node_fs14.default.existsSync(candidate)) return candidate;
    }
  } catch {
  }
  return null;
...
async function main() {
  await runHook({
    ...
    handler: async (ctx) => {
      ...
      const sessionsDir = import_node_path12.default.join(home, ".teamagent", "sessions");

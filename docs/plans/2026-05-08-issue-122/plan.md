...
| **C** Hero GIF (closes #120 too) | sonnet worker C | Drop `apps/landing/public/double-moment.gif` (use existing recording from #120 work, or re-record via `asciinema` + `agg`). Replace `.gif-placeholder` div in `apps/landing/src/index.html` with `<img src="/TeamBrain/double-moment.gif" alt="…" loading="lazy" />`. | `apps/landing/public/double-moment.gif`, `apps/landing/src/index.html` (one block swap). |

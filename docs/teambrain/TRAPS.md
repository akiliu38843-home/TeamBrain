...
| TRAP-OPS-011 | ops | P1 | task closed without saving `.judge/` evidence artifacts | every task must save `stdout.txt`, `coverage.json`, `judge.json` to `.judge/<run_id>/` before committing; verbal "evidence" is rejected | `h=$(git log -1 --pretty=%h); test -d ".judge/$h" && test -s ".judge/$h/judge.json" \|\| exit 1` | docs/notes/2026-05-01-day0-team-experience-dump.md §C trap #30 |

...
function defaultRunner(env: Record<string, string | undefined>, cwd: string): DocsPropagationRunner {
  const command = env.TEAMAGENT_DOCS_RUNNER ?? "/Users/liushiyu/.local/bin/claudefast -p";
...
function runPromptCommand(
  ...
  const command = commandTemplate.includes("{prompt}")
    ? commandTemplate.replaceAll("{prompt}", shellQuote(prompt))
    : `${commandTemplate} ${shellQuote(prompt)}`;
...
export async function executeDocsPropagate(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (attemptLog.checks.length === entries.length && attemptLog.checks.every((c) => c.pass)) {
      result.ok = true;
      break;
    }
  }
